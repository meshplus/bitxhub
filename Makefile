...
TEST_PKGS2 := $(shell $(GO) list ./... | grep -v 'pkg' |grep -v 'asset_manager.go' | grep -v 'rule_manager.go' | grep -v "governance.go" |grep -v 'node_manager.go' | grep 'handle.go' | grep -v 'ledger' | grep -v 'appchain' | grep -v 'repo' | grep -v 'router' | grep -v 'mock_*' | grep -v 'tester' | grep -v 'proto' | grep -v 'cmd'| grep -v 'api')
