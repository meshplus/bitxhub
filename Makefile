...
TEST_PKGS2 := $(shell $(GO) list ./... | grep -v 'ratelimiter' | grep -v 'vm' | grep -v 'peermgr' | grep -v 'repo' | grep -v 'router' | grep -v 'mock_*' | grep -v 'tester' | grep -v 'proto' | grep -v 'cmd'| grep -v 'api')
