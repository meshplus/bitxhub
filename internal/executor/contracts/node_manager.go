...
func (nm *NodeManager) Manage(eventTyp, proposalResult, lastStatus, objId string, extra []byte) *boltvm.Response {
	...
	if err != nil {
		return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("marshal specificAddrs error: %v", err))
	}
	...
	if !ok {
		return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("change status error: %s", string(errData)))
	}

	node, err := nm.NodeManager.QueryById(objId, nil)
	if err != nil {
		return boltvm.Error(boltvm.NodeInternalErrCode, err.Error())
	}
	nodeInfo := node.(*nodemgr.Node)

	// 3. other operation
	if proposalResult == string(APPROVED) {
		if bvmErr := nm.processApproveProposal(eventTyp, nodeInfo, objId, extra); !bvmErr.Ok {
			return bvmErr
		}
	} else {
		if bvmErr := nm.processRejectProposal(eventTyp, nodeInfo, extra); !bvmErr.Ok {
			return bvmErr
		}
	}
	...
	if nm.EnableAudit() {
		if err = nm.postAuditNodeEvent(objId); err != nil {
			return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("post audit node event error: %v", err))
		}
	}
	return boltvm.Success(nil)
}

func (nm *NodeManager) processApproveProposal(eventTyp string, nodeInfo *nodemgr.Node, objId string, extra []byte) *boltvm.Response {
	switch eventTyp {
	...
	case string(governance.EventUpdate):
		...
		if err := json.Unmarshal(extra, updateInfo); err != nil {
			return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("unmarshal update data error:%v", err))
		}
		...
		if ok, data := nm.NodeManager.Update(updateNode); !ok {
			return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("update node error: %s", string(data)))
		}
		...
		if nodeInfo.NodeType == nodemgr.NVPNode && nodeInfo.AuditAdminAddr != "" {
			...
			if !res.Ok {
				return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("cross invoke IsAnyAvailableAdmin error: %s", string(res.Result)))
			}
...
func (nm *NodeManager) processRejectProposal(eventTyp string, nodeInfo *nodemgr.Node, extra []byte) *boltvm.Response {
	switch eventTyp {
	case string(governance.EventRegister):
		if res := nm.CrossInvoke(constant.RoleContractAddr.String(), "FreeAccount", pb.String(nodeInfo.Account)); !res.Ok {
			return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("cross invoke FreeAccount error: %s", string(res.Result)))
		}
		switch nodeInfo.NodeType {
		case nodemgr.VPNode:
	...
	case string(governance.EventUpdate):
		...
		if err := json.Unmarshal(extra, nodeUpdateInfo); err != nil {
			return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("unmarshal node error: %v", err))
		}
	...
	case string(governance.EventLogout):
		if nodeInfo.NodeType == nodemgr.NVPNode {
			if nodeInfo.AuditAdminAddr != "" {
				...
				if !res.Ok {
					return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("cross invoke GetRoleInfoById error: %s", string(res.Result)))
				}
				role := &Role{}
				if err := json.Unmarshal(res.Result, role); err != nil {
					return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("unmarshal role error: %v", err))
				}
				...
				if role.Status == governance.GovernanceForbidden {
					...
					if res := nm.CrossInvoke(constant.RoleContractAddr.Address().String(), "RestoreAuditAdminBinding", pb.String(nodeInfo.Account)); !res.Ok {
						return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("cross invoke RestoreAuditAdminBinding error: %s", string(res.Result)))
					}
				}
			}
		}
...
func (nm *NodeManager) RegisterNode(nodeAccount, nodeType, nodePid string, nodeVpId uint64, nodeName, permitStr, reason string) *boltvm.Response {
	...
	if nodeType == string(nodemgr.VPNode) && nm.hasVpNodeGoverned() {
		return boltvm.Error(boltvm.NodeVPBeingGovernedCode, string(boltvm.NodeVPBeingGovernedMsg))
	}
	...
	if res := nm.CrossInvoke(constant.RoleContractAddr.String(), "OccupyAccount", pb.String(nodeAccount), pb.String(string(NodeAccount))); !res.Ok {
		return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("cross invoke OccupyAccount error: %s", string(res.Result)))
	}
	...
	if !res.Ok {
		return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("submit proposal error: %s", string(res.Result)))
	}
...
//   - binding(only nvp):
//     when submitting a proposal, 1. need to pause the proposal for the role registration or binding
//     when proposal approved,
//     if bound role is still registering or binding, 1. need to reject the proposal for the role registration or binding; 2. pause the role (going into the unavailable or frozen state)
...
//     if bound role is still registering or binding, 1. need to restore the proposal for the role registration or binding
//     if bound role is forbidden, 1. need to reject the proposal for the role registration or binding;
//   - binded(only nvp):
//     when submitting a proposal, no additional operations
//     when proposal approved,
//     if bound role is available, 1. need to pause the role (going into the frozen state)
//     if bound role is forbidden, 1. need to pause the role (not change state)
//     when proposal rejected,
//     if bound role is available, no additional operation
...
func (nm *NodeManager) LogoutNode(nodeAccount, reason string) *boltvm.Response {
	...
	if node.NodeType == nodemgr.VPNode {
		...
		if !ok {
			return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("count available nodes error: %s", string(data)))
		}

		vpNum, err := strconv.Atoi(string(data))
		if err != nil {
			return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("get vp node num error: %v", err))
		}
	...
	if ok, data := nm.NodeManager.ChangeStatus(nodeAccount, string(event), string(node.Status), nil); !ok {
		return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("change status error: %s", string(data)))
	}
	...
	if node.Status == governance.GovernanceBinding {
		if res := nm.CrossInvoke(constant.RoleContractAddr.Address().String(), "PauseAuditAdminBinding", pb.String(nodeAccount)); !res.Ok {
			return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("cross invoke PauseAuditAdminBinding error: %s", string(res.Result)))
		}
	}
...
func (nm *NodeManager) UpdateNode(nodeAccount, nodeName, permitStr, reason string) *boltvm.Response {
	...
	if len(node.Permissions) != len(newNode.Permissions) {
		...
		for permit := range newNode.Permissions {
	...
	if err != nil {
		return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("marshal updateNodeInfo error: %v", err))
	}
...
func (nm *NodeManager) BindNode(nodeAccount, auditAdminAddr string) *boltvm.Response {
	...
	if ok, data := nm.NodeManager.Bind(nodeAccount, auditAdminAddr); !ok {
		return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("record bind admin error: %s", string(data)))
	}
...
func (nm *NodeManager) GetNvpNodeByName(nodeName string) *boltvm.Response {
	...
	node, err := nm.NodeManager.QueryById(nodeAccount, nil)
	if err != nil {
		return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("node name %s exist but node %s not exist: %v", nodeName, nodeAccount, err))
	}
	if data, err := json.Marshal(node.(*nodemgr.Node)); err != nil {
		return boltvm.Error(boltvm.NodeInternalErrCode, err.Error())
	} else {
...
func (nm *NodeManager) GetVpNodeByPid(nodePid string) *boltvm.Response {
	...
	node, err := nm.NodeManager.QueryById(nodeAccount, nil)
	if err != nil {
		return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("node pid %s exist but node %s not exist: %v", nodePid, nodeAccount, err))
	}
	if data, err := json.Marshal(node.(*nodemgr.Node)); err != nil {
		return boltvm.Error(boltvm.NodeInternalErrCode, err.Error())
	} else {
...
func (nm *NodeManager) GetVpNodeByVpId(nodeVpId string) *boltvm.Response {
	...
	node, err := nm.NodeManager.QueryById(nodeAccount, nil)
	if err != nil {
		return boltvm.Error(boltvm.NodeInternalErrCode, fmt.Sprintf("node vpId %s exist but node %s not exist: %v", nodeVpId, nodeAccount, err))
	}
	if data, err := json.Marshal(node.(*nodemgr.Node)); err != nil {
		return boltvm.Error(boltvm.NodeInternalErrCode, err.Error())
	} else {
...
func (nm *NodeManager) checkNodeInfo(node *nodemgr.Node, isRegister bool) *boltvm.Response {
	...
	case nodemgr.NVPNode:
		...
		if len(node.Permissions) == 0 {
			return boltvm.Error(boltvm.NodeEmptyPermissionCode, string(boltvm.NodeEmptyPermissionMsg))
		}
