...
func (tlm *txLiveTimeMap) insertByTtlKey(account string, nonce uint64, liveTime int64) {
	tlm.index.ReplaceOrInsert(&orderedTimeoutKey{account: account, nonce: nonce, timestamp: liveTime})
...
func (tlm *txLiveTimeMap) removeByTtlKey(txs map[string][]*types.Transaction) {
	for account, list := range txs {
		for _, tx := range list {
			...
			tlm.index.Delete(&orderedTimeoutKey{account: account, nonce: tx.GetNonce(), timestamp: liveTime})
...
func (tlm *txLiveTimeMap) insertOrUpdateByTtlKey(account string, nonce uint64, liveTime int64) {
	...
	if oldTime, ok := tlm.items[accountNonceKey]; ok {
		tlm.index.Delete(&orderedTimeoutKey{account: account, nonce: nonce, timestamp: oldTime})
	}
...
func (tam *txArrivedTimeMap) insertByTtlKey(account string, nonce uint64, liveTime int64) {
	tam.index.ReplaceOrInsert(&orderedTimeoutKey{account: account, nonce: nonce, timestamp: liveTime})
...
func (tam *txArrivedTimeMap) removeByTtlKey(txs map[string][]*types.Transaction) {
	for account, list := range txs {
		for _, tx := range list {
			...
			tam.index.Delete(&orderedTimeoutKey{account: account, nonce: tx.GetNonce(), timestamp: liveTime})
...
func (tam *txArrivedTimeMap) insertOrUpdateByTtlKey(account string, nonce uint64, liveTime int64) {
	...
	if oldTime, ok := tam.items[accountNonceKey]; ok {
		tam.index.Delete(&orderedTimeoutKey{account: account, nonce: nonce, timestamp: oldTime})
	}
