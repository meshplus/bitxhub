...
print_blue "===> 1. Check go.mod"
noTagPackages=$(grep ".*\s.*" < "$PROJECT_PATH"/go.mod | grep -v require | grep -v indirect | grep -v replace | grep -v // | grep -c "v\d\+\.\d\+\.\d\+-")
if [ "$noTagPackages" -gt 0 ]; then
  print_red "Repository has packages without tag, please check go.mod"
  exit 1
fi
